import { useState } from "react";
function ResponseRatingType({ questionId, onResponseChange, value }) {
  const [starCount, setStarCount] = useState(value || 0);
  const handleStarClick = (e) => {
    const clickedStarCount = parseInt(e.currentTarget.id);
    setStarCount(clickedStarCount);
    if (onResponseChange) {
      onResponseChange(questionId, clickedStarCount);
    }
  };
  return (
    <div className="rating-type-container">
      <div className="start-icon-container">
        {[1, 2, 3, 4, 5].map((num) => (
          <svg
            key={num}
            width="28"
            height="26"
            viewBox="0 0 32 31"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            id={num}
            onClick={handleStarClick}
            style={{ cursor: "pointer" }}
          >
            <path
              d="M15.0489 0.927048C15.3483 0.00573707 16.6517 0.00574017 16.9511 0.927051L20.0413 10.4377C20.1751 10.8497 20.5591 11.1287 20.9923 11.1287H30.9924C31.9611 11.1287 32.3639 12.3683 31.5802 12.9377L23.4899 18.8156C23.1395 19.0702 22.9928 19.5216 23.1267 19.9336L26.2169 29.4443C26.5162 30.3656 25.4617 31.1317 24.678 30.5623L16.5878 24.6844C16.2373 24.4298 15.7627 24.4298 15.4122 24.6844L7.32198 30.5623C6.53826 31.1317 5.48378 30.3656 5.78314 29.4443L8.87333 19.9336C9.00721 19.5216 8.86055 19.0702 8.51006 18.8156L0.419821 12.9377C-0.363892 12.3683 0.0388863 11.1287 1.00761 11.1287H11.0077C11.4409 11.1287 11.8249 10.8497 11.9587 10.4377L15.0489 0.927048Z"
              fill={num <= starCount ? "#69B5F8" : "#D9D9D9"}
            />
          </svg>
        ))}
      </div>
      <div className="rating-text-container">
        <p>Star Count :</p>
        <p className="star-count">{starCount}</p>
      </div>
    </div>
  );
}
export default ResponseRatingType;



