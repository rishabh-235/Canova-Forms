...
function FormCard({ form }) {
  ...
  const handleOptionClick = async (e) => {
    ...
    const sharedWithUser = form.sharedWith.find(
      (shared) => shared.email === user.email
    );
    const userPermissionType = sharedWithUser?.type;

    if (action === "Share") {
      if (
        !isCreator &&
        (!sharedWithUser || !["edit", "share"].includes(userPermissionType))
      ) {
        toast.error("You don't have permission to share this form");
        setToggleOptions(!toggleOptions);
        return;
      }
      setToggleShareCard(!toggleShareCard);
    } else if (action === "Rename") {
      if (!isCreator && (!sharedWithUser || userPermissionType !== "edit")) {
        toast.error("You don't have permission to rename this form");
        setToggleOptions(!toggleOptions);
        return;
      }
  ...
  const handleSaveFormName = (e) => {
    ...
    // Check if user is in sharedWith array and get their permission type
    const sharedWithUser = form.sharedWith.find(
      (shared) => shared.email === user.email
    );
    const userPermissionType = sharedWithUser?.type;

    // Only creator or users with 'edit' permission can rename
    if (!isCreator && (!sharedWithUser || userPermissionType !== "edit")) {
      toast.error("You don't have permission to rename this form");
  ...
  const handleViewForm = () => {
    ...
    if (isCreator) {
      // If user is the creator, they can edit the form
      navigate(`/create-form/${form._id}/1`);
      return;
    }

    // If not creator, check sharedWith permissions
    const shareWith = form.sharedWith.find(
      (email) => email.email === user.email
    );
    if (shareWith) {
      if (shareWith.type === "edit") {
        navigate(`/create-form/${form._id}/1`);
      } else if (shareWith.type === "view") {
        navigate(`/response-form/${form._id}/1`);
      }
    } else {
      // User has no permission to view this form
      ...
      <div className="form-card-footer">
        ...
        <>
          ...
          <div
            ...
            className={`form-card-options ${toggleOptions ? "show" : ""}`}
          >
            <button>Share</button>
